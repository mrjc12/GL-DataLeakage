...
class EnsembleTrainer():
    ...
    def _evaluate(self, logits):
        if self.multi_label:
            val_acc = self._evaluator.eval({"y_pred": logits[self.data.val_mask], "y_true": self.data.y[self.data.val_mask]})
            test_acc = self._evaluator.eval({"y_pred": logits[self.data.test_mask], "y_true": self.data.y[self.data.test_mask]})
        else:
    ...
    def eval(self, logits):
        ...
        if self.multi_label:
            print(
                f'({self.feature_type}) Ensemble ValMicro: {val_acc["micro_f1"]:.4f}, ValMacro: {val_acc["macro_f1"]:.4f}, TestMicro: {test_acc["micro_f1"]:.4f}, TestMacro: {test_acc["macro_f1"]:.4f}\n')
            ...
            print(
                f'({self.feature_type}) Val: {val_acc:.4f}, Test: {test_acc:.4f}\n')
