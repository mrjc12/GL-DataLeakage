...
from ai.api import HongLou_batch_jobs
from ai.prompts import HongLou_prompt


def get_all_raw_HongLou(dataset_path, text_select):
    items = []
...
def generate_HongLou(dataset_path, save_path, text_select, llm='gpt-3.5-turbo'):
    completed_ids = set()
    ...
    raw_items = get_all_raw_HongLou(dataset_path, text_select)
    all_prompts = HongLou_prompt(raw_items)
    ...
    asyncio.run(HongLou_batch_jobs(tasks, save_path))

if __name__ == '__main__':
    DATASET_FILE = 'dataset/HongLou/nodes.csv'
    LLM_MODEL = 'gpt'
    TEXT_SELECT = 'N_DES'

    SAVE_DIR = 'gpt_response/HongLou'
